...
req = cursor.execute("SELECT * FROM raw_data_pressure")
res = req.fetchall()
...
df.drop([i for i in col_ratio_init if "per_match" not in i], axis = 1, inplace = True)
...
df.to_sql(name = "Pressure", con = connect, if_exists = "replace", index = False)
