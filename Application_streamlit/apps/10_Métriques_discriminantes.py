...
if len(liste_saison) > 0 :
    ...
    for saison in liste_saison :
        ...
        if choix_data == "Skill Corner" :
            ...
            nb_matchs_team = df_metrique.groupby("team_name").apply(len).reindex(dico_rank_SK[saison])

            df_metrique = df_metrique.reset_index().drop(["Journée", "result"], axis = 1).groupby("team_name", as_index = True, sort = False).sum().reindex(dico_rank_SK[saison])
