...
from config_py_BDD.fonction import func_change, execute_SQL
from config_py_BDD.variable import dico_type, dico_rank_SK, dico_rank_SB, dico_cat_run, dico_cat_met_pressure, dico_type_passe
...
with col1 :
    ...
    liste_saison = [i[0].replace("_", "/") for i in liste_saison]
    choix_saison = st.multiselect("Choisir saison", liste_saison, default = max(liste_saison))
...
with columns[1] :
    ...
    if nb_top == max_team :
        ...
        nb_bottom = st.slider("Nombre d'équipe dans le Bottom", min_value = 0, max_value = max_team - nb_top)
...
with col2 :
    ...
    for saison in choix_saison :
        ...
        df_metrique.loc[saison, liste_rank[:nb_top], "Groupe"] = "Top"
        df_metrique.loc[saison, liste_rank[nb_top:nb_top + nb_middle.loc[saison]], "Groupe"] = "Middle"
        df_metrique.loc[saison, liste_rank[nb_top + nb_middle.loc[saison]:], "Groupe"] = "Top"

    st.write(df_metrique.groupby("Groupe").mean())
