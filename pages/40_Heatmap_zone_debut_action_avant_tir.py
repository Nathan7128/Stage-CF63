...
with columns[0] :
    annee = st.selectbox("Choisir année", options = ["2020_2021", "2021_2022", "2022_2023", "2023_2024"])
...
with columns[1] :
    ...
    type_action = st.multiselect("Choisir le type de début d'action", options = df.type_action.unique(),
                                 default = df.type_action.unique())
...
if len(type_action) > 0 :
    ...
    def heatmap_percen(data, bins_h, bins_v, choix_percent, choix_line) :
        ...
        pitch = VerticalPitch(pitch_type='statsbomb', line_zorder=2, pitch_color='#f4edf0', line_color = "#f4edf0", goal_type = "box",
                            linewidth = 1*(1 - choix_line), spot_scale = 0.002*(1 - choix_line))
