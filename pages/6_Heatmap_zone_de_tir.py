...
df = pd.read_excel(f"Heatmap SB/zone_tir/Tableaux/{annee}/zone_tir.xlsx", index_col = 0)
...
# col1, col2, col3 = st.columns([1, 3, 3], vertical_alignment = "center")
# with col1 :
#     slider_bin_v = svs.vertical_slider(step = 1, height = 370, min_value = 0, max_value = bins_v - 1)
# with col2 :
#     heatmap_percen(df_sort, bins_h, bins_v)
# with col3 :
#     heatmap_smooth(df_sort)

# with st.columns([1, 3, 3, 3, 3], vertical_alignment = "center")[2] :
#     slider_bin_h = st.slider("choix", step = 1, min_value = 0, max_value = bins_h - 1, label_visibility = "hidden")